...
def main():
    ...
    # load integrations to mask
    if args.mask_file != '':
        slices = read_masking_slices(args.mask_file)
    else:
        slices = [[(0, 0)] for f in files]
    ...
    control_dict = {
        ...
        'start_wav': start_wav,
        'end_wav': end_wav,
        'samples': 100, # total number of MCMC samples per chain
        'burnin': 20, # number of samples to discard from start of chains 
        'num_proc': args.nproc,
        ...
        'mask_slices': slices # integrations with indices between each set of numbers are masked before fitting
    }

    # prepare the data by extracting arrays from .h5 files and computing the expected transit times 
    times, spects, errs, _, specs, flux, _ = prep_data(control_dict)
    ...
    for i, res in enumerate(wlc_res):
        ...
        axs[0].plot(
            res['time'][res['mask']], 
            res['flux'][res['mask']], 
            '.', color=plt.cm.rainbow(0.0), markersize=2.0
        )
        axs[0].plot(
            res['time'][~res['mask']], 
            res['flux'][~res['mask']], 
            '.', color=plt.cm.rainbow(0.0), markersize=2.0, alpha=0.2
        )
        [axs[0].plot(res['time'], m, color='k', alpha=0.3) for m in models];
        axs[1].plot(
            res['time'][res['mask']], 
            res['flux'][res['mask']] - np.mean(models, axis=0), 
            '.', color=plt.cm.rainbow(0.0), markersize=2.0
        )
        axs[1].plot(
            res['time'][~res['mask']], 
